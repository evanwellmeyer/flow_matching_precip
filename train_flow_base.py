...
def main():
    ...
    # AMIP models (real trends, one member each)
    for clim_path in sorted(AMIP_DIR.glob("AMIP_*_clim.zarr")):
        model      = clim_path.name.replace("AMIP_", "").replace("_clim.zarr", "")
        trend_path = AMIP_DIR / f"AMIP_{model}_trend.zarr"
        dpdp_path  = AMIP_DIR / f"AMIP_{model}_dPdP.zarr"
        if not trend_path.exists() or not dpdp_path.exists():
            print(f"  WARN AMIP {model}: missing trend or dPdP, skipping")
            continue
        try:
            clim  = load_zarr_field(clim_path,  "pr_clim")
            trend = load_zarr_field(trend_path, "pr_trend")
            dpdp  = load_zarr_field(dpdp_path,  "dPdP")
